...
int main(int, char**)
{
    ...
    auto http_requests_family = registry->create_metric<tc::prometheus::counter>(
        "http_requests_total",
    ...
    auto database_queries_family = registry->create_metric<tc::prometheus::counter>(
    ...
    auto cache_hits_family = registry->create_metric<tc::prometheus::counter>(
        "cache_hits_total",
        "Total number of cache hits");

    auto cache_hits = cache_hits_family->add_metric({{"cache_type", "redis"}});

    auto cache_misses_family = registry->create_metric<tc::prometheus::counter>(
        "cache_misses_total",
    ...
    // 3. Simulate realistic application behavior
    for (int i = 0; i < 100; ++i)
    {
